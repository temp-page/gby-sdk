...
let Erc20Service = class Erc20Service extends BaseService_1.BaseService {
    ...
    async getBalance(address, user) {
        ...
        const [result] = await this.connectInfo.multiCall().callObj({
            balance: tokenIns.multicall_balanceOf(user),
            decimals: tokenIns.multicall_decimals(),
        });
    ...
    async getTokenInfo(address) {
        const tokenIns = this.connectInfo.create(abi_1.ERC20Contract, address);
        const [result] = await this.connectInfo.multiCall().callObj({
            name: tokenIns.multicall_name(),
            symbol: tokenIns.multicall_symbol(),
            decimal: tokenIns.multicall_decimals(),
    ...
    async getAllowance(exchangeAddress, tokenAddress, userAddress) {
        ...
        const [result] = await this.connectInfo.multiCall().callObj({
            allowance: tokenIns.multicall_allowance(userAddress, exchangeAddress),
            decimals: tokenIns.multicall_decimals(),
        });
    ...
    async batchGetBalance(user, tokens) {
        ...
        const result = await this.connectInfo.multiCall().callObj(...tokens.map((it) => {
            if (it === vo_1.ETH_ADDRESS) {
                return {
                    address: vo_1.ETH_ADDRESS,
                    balance: multiCall.multicall_getEthBalance(user),
                    decimals: '18',
                };
            }
    ...
    async batchGetBalanceAndAllowance(user, spender, tokens) {
        ...
        const result = await this.connectInfo.multiCall().callObj(...tokens.map((it) => {
            ...
            return {
                address: it.address,
                balance: tokenIns.multicall_balanceOf(user),
                allowance: tokenIns.multicall_allowance(user, spender),
                decimals: Number(it.decimals).toFixed(),
            };
        }));
        const data = {};
