...
function mixProxyByConnect(clazz, connectInfo, ...args) {
    ...
    try {
        const chainType = connectInfo ? connectInfo.chainInfo().chainType : "NONE";
        const cacheType = connectInfo && connectInfo.writeState ? "CONNECT_INFO" : "MIX_PROXY";
        const key = `${cacheType}_${chainType}_${cacheKey}_${JSON.stringify(args)}`;
        ...
        get(target, propKey) {
            ...
            if (ins && (ins.proxyEnable || ins.logEnable || ins.methodCache)) {
                return function () {
                    ...
                    const showLog = (data) => {
                        if (ins.logEnable) {
                            if (data instanceof vo_1.TransactionEvent) {
                                data = {
                                    hash: data.hash(),
                                    account: data.connectInfo.account,
                                    addressInfo: data.connectInfo.addressInfo
                                };
                            }
