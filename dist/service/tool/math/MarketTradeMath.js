...
class MarketTradeMath {
    ...
    static notionalPositionValue(tokenAmount, leverage, tokenPrice, decimals) {
        return new bignumber_js_1.default(tokenAmount)
    ...
    // uint256 openFee = (notionalPositionValue * targetConfig.openFeeRate) / 10 ** CONST.RATIO_DECIMALS;
    static openFee(notionalPositionValue, openFeeRate) {
        return new bignumber_js_1.default(notionalPositionValue)
    ...
    //openTrade.isBoost
    //             ? (notionalPositionValue * targetConfig.closeFeeRate) / 10 ** CONST.RATIO_DECIMALS
    //             : 0;
    static closeFee(isBoost, notionalPositionValue, closeFeeRate) {
        if (isBoost) {
            return new bignumber_js_1.default(notionalPositionValue)
    ...
    //uint256 totalFee = openFee + closeFee + targetConfig.execFeeValue;
    static totalFee(openFee, closeFee, execFeeValue) {
    ...
    static initTargetFundingFeeRate(targetTick, getTargetConfig) {
        //fundingFeePerBlockTimeP * 1e18 * 1 * 60 * 60  / 1e18 / 1e18
        const fundingRate = new bignumber_js_1.default(getTargetConfig.fundingFeePerBlockP).multipliedBy(1e18).multipliedBy(1).multipliedBy(60).multipliedBy(60).div(1e18).div(1e18).toFixed();
        //  fundingRateLong =fundingRate  *  （LongOI  - ShortOI） / LongIO
        const fundingRateLong = new bignumber_js_1.default(targetTick.longOI).eq("0") ? "0" : new bignumber_js_1.default(fundingRate).multipliedBy(new bignumber_js_1.default(targetTick.longOI).minus(new bignumber_js_1.default(targetTick.shortOI))).div(new bignumber_js_1.default(targetTick.longOI)).multipliedBy(100).toFixed();
        //  fundingRateShort =fundingRate  *  （ShortOI  - LongOI） / ShortOI
        const fundingRateShort = new bignumber_js_1.default(targetTick.shortOI).eq("0") ? "0" : new bignumber_js_1.default(fundingRate).multipliedBy(new bignumber_js_1.default(targetTick.shortOI).minus(new bignumber_js_1.default(targetTick.longOI))).div(new bignumber_js_1.default(targetTick.shortOI)).multipliedBy(100).toFixed();
