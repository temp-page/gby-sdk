...
let CompetitionApi = class CompetitionApi {
    async list() {
        ...
        return {
            list: competitionList.map((it, index) => {
                let status = 'Open';
                ...
                return {
                    ...
                    endTime: it.endTime,
                    status: status,
                    current: it.startTime <= Date.now() && it.endTime >= Date.now() || index === 9 && it.startTime > Date.now() || index === 0 && it.endTime < Date.now(),
                };
            }),
        };
    }
    async claimInfo(address) {
        if (!address) {
            throw new Error("Address is required to get competition claim info.");
        }
        ...
        const tradeRewardsContract = !claimInfo.claimInfo ? undefined : ci.create(contract_1.TradeRewardsContract, claimInfo.claimInfo.contract);
        let claimed = false;
        let claimShowStatus = '';
        if (tradeRewardsContract && claimInfo.claimInfo) {
            let [result] = await ci.multiCall().callObj({
                getClaimed: tradeRewardsContract.multicall.getClaimed(claimInfo.claimInfo.round, claimInfo.claimInfo.address),
                rewards: tradeRewardsContract.multicall.rewards(claimInfo.claimInfo.round),
                getCurrentBlockTimestamp: ci.multiCall().multicall2Contract.multicall.getCurrentBlockTimestamp()
            });
            let expired = new bignumber_js_1.default(result.rewards.deadline).comparedTo(result.getCurrentBlockTimestamp) < 0;
            claimed = result.getClaimed || expired;
            if (result.getClaimed) {
                claimShowStatus = 'claimed';
            }
            else if (expired) {
                claimShowStatus = 'expired';
            }
        }
        return {
            address: address,
            rewardSymbol: (0, get_1.default)(claimInfo, 'claimInfo.rewardSymbol', ''),
            reward: new bignumber_js_1.default((0, get_1.default)(claimInfo, 'claimInfo.showReward', '0')).toFixed(),
            ...
            claim: async (connectInfo) => {
                return await connectInfo.create(contract_1.TradeRewardsContract, claimInfo.claimInfo.contract).claim(claimInfo.claimInfo.round, claimInfo.claimInfo.reward, claimInfo.claimInfo.proof);
            }
        };
    }
        ...
        return {
            ...
            filter(current, size, search, type) {
                ...
                if ((0, tool_1.isNullOrBlank)(search)) {
                    ...
                    return filteredList.slice((current - 1) * size, (current) * size);
                }
            }
        };
    }
