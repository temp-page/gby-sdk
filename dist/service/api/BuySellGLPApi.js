...
let BuySellGLPApi = class BuySellGLPApi {
    ...
    async tokens(chainType, account = undefined) {
        ...
        const chainInfo = addressInfo.getChainInfo(chainType);
        const connectInfo = BaseApi_1.BASE_API.connectInfo(chainInfo);
        const glpContract = connectInfo.create(contract_1.IERC20Contract, chainInfo.GLP);
        ...
        const [glp, ...tokenConfigs] = await connectInfo.multiCall().callObj({
            totalSupply: glpContract.multicall.totalSupply(),
            decimals: glpContract.multicall.decimals(),
        }, ...vaultTokens.map(it => {
            return {
        ...
        return vaultTokens.map((it, index) => {
            ...
            let taxBasisPoints = "0";
            {
                ...
                const vaultValue = vaultTokens.map((it, index) => {
                    ...
                    return tool_1.VaultServiceMath.getVaultValue(it.token, tokenAmount, tokenPrice);
                }).reduce((a, b) => new bignumber_js_1.default(a).plus(b).toFixed(0), "0");
                const inputValue = new bignumber_js_1.default("100").multipliedBy(10 ** it.token.decimals).toFixed(0, bignumber_js_1.default.ROUND_DOWN);
                const tokenValue = tool_1.VaultServiceMath.getVaultValue(it.token, inputValue, tokenPrice);
                const tokenValueInVault = tool_1.VaultServiceMath.getVaultValue(it.token, tokenConfigs[vaultTokenIndex].tokenAmount, tokenPrice);
                const feePoint = tool_1.LPServiceMath.getGlpFeePoint(tokenValue, vaultValue, tokenValueInVault, true, tokenConfigs[vaultTokenIndex].tokenConfig, vaultTokens[vaultTokenIndex].weight);
            ...
            }
            {
                ...
                const amountGLP = new bignumber_js_1.default("100").multipliedBy(10 ** parseInt(glp.decimals)).toFixed(0, bignumber_js_1.default.ROUND_DOWN);
                const alpPrice = tool_1.LPServiceMath.getGlpPrice(vaultValue, glp.totalSupply);
                ...
                const tokenValueInVault = tool_1.VaultServiceMath.getVaultValue(it.token, tokenConfigs[vaultTokenIndex].tokenAmount, tokenPrice);
                const feePoint = tool_1.LPServiceMath.getGlpFeePoint(valueGLP, vaultValue, tokenValueInVault, false, tokenConfigs[vaultTokenIndex].tokenConfig, vaultTokens[vaultTokenIndex].weight);
    ...
    async buyInfo(chainType, token, account = tool_1.INVALID_ADDRESS) {
        ...
        const [glp, ...prices] = await connectInfo.multiCall().callObj({
            totalSupply: glpContract.multicall.totalSupply(),
            decimals: glpContract.multicall.decimals(),
            balance: glpContract.multicall.balanceOf(account ? account : tool_1.INVALID_ADDRESS),
        }, ...vaultTokens.map(it => {
            return {
                price: pythApi.priceBySymbol(it.token.symbol),
                tokenAmount: vaultServiceContract.multicall.getAmountInLP(it.address),
        ...
        const preBuy = async (inputAmount, fee = true) => {
            ...
            const tokenValue = tool_1.VaultServiceMath.getVaultValue(token, inputValue, tokenPrice);
            const tokenValueInVault = tool_1.VaultServiceMath.getVaultValue(token, prices[vaultTokenIndex].tokenAmount, tokenPrice);
            const feePoint = fee ? tool_1.LPServiceMath.getGlpFeePoint(tokenValue, vaultValue, tokenValueInVault, true, prices[vaultTokenIndex].tokenConfig, vaultTokens[vaultTokenIndex].weight) : 0;
            // TODO not impl
            const apy = "10%";
        ...
        const buy = async (inputAmount, connectInfo) => {
            ...
            return await connectInfo.create(contract_1.LpServiceContract).mintALP(vaultToken.assetID, inputValue, min, priceFeedsUpdateData, value);
        };
    ...
    async sellInfo(chainType, token, account = tool_1.INVALID_ADDRESS) {
        ...
        const glpContract = connectInfo.create(contract_1.IERC20Contract, chainInfo.GLP);
        const vaultServiceContract = connectInfo.create(contract_1.VaultServiceContract);
        const lpServiceContract = connectInfo.create(contract_1.LpServiceContract);
        ...
        const [glp, ...prices] = await connectInfo.multiCall().callObj({
            ...
            accountLatestMintTime: lpServiceContract.multicall.getAccountLatestMintTime(account),
            coolingDuration: lpServiceContract.multicall.getCoolingDuration(),
        }, ...vaultTokens.map(it => {
            return {
                price: pythApi.priceBySymbol(it.token.symbol),
                tokenAmount: vaultServiceContract.multicall.getAmountInLP(it.address),
        ...
        const preSell = async (inputAmount = "0", fee = true) => {
            ...
            const valueGLP = new bignumber_js_1.default(amountGLP).multipliedBy(alpPrice).div(1e8).toFixed(0, bignumber_js_1.default.ROUND_DOWN);
            const tokenValueInVault = tool_1.VaultServiceMath.getVaultValue(token, prices[vaultTokenIndex].tokenAmount, tokenPrice);
            const feePoint = fee ? tool_1.LPServiceMath.getGlpFeePoint(valueGLP, vaultValue, tokenValueInVault, false, prices[vaultTokenIndex].tokenConfig, vaultTokens[vaultTokenIndex].weight) : 0;
        ...
        const sell = async (inputAmount, connectInfo) => {
            ...
            const priceFeedsUpdateData = await ApiProvider_1.apiProvider.pythApi().priceFeedsUpdateDataByTokens(vaultTokens.map(it => it.token.symbol));
            return await connectInfo.create(contract_1.LpServiceContract).burnALP(vaultToken.assetID, inputValue, min, priceFeedsUpdateData);
        };
