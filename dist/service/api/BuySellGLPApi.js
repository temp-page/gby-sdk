...
let BuySellGLPApi = class BuySellGLPApi {
    ...
    async buyInfo(chainType, token, account = tool_1.INVALID_ADDRESS) {
        ...
        const [glp, ...prices] = await connectInfo.multiCall().call({
            totalSupply: glpContract.mulContract.totalSupply(),
            decimals: glpContract.mulContract.decimals(),
            balance: glpContract.mulContract.balanceOf(account ? account : tool_1.INVALID_ADDRESS),
        }, ...vaultTokens.map(it => {
            return {
    ...
    async sellInfo(chainType, token, account = tool_1.INVALID_ADDRESS) {
        ...
        const [glp, ...prices] = await connectInfo.multiCall().call({
            totalSupply: glpContract.mulContract.totalSupply(),
            decimals: glpContract.mulContract.decimals(),
            blockTime: multiCallContract.mulContract.getCurrentBlockTimestamp(),
            accountLatestMintTime: lpServiceContract.mulContract.getAccountLatestMintTime(account),
            coolingDuration: lpServiceContract.mulContract.getCoolingDuration(),
        }, ...vaultTokens.map(it => {
            return {
                price: portalContract.mulContract.getPrice(it.assetID),
                tokenAmount: vaultServiceContract.mulContract.getTokenAmount(it.address)
