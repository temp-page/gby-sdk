...
let BuySellGLPApi = class BuySellGLPApi {
    ...
    async buyInfo(chainType, token, account = tool_1.INVALID_ADDRESS) {
        ...
        const preBuy = async (inputAmount) => {
            ...
            const feePoint = abi_1.LPServiceContract.getGlpFeePoint(tokenValue, vaultValue, tokenValueInVault, true, prices[vaultTokenIndex].tokenConfig, vaultTokens[vaultTokenIndex].weight);
            // TODO not impl
            const apy = "10%";
    ...
    async sellInfo(chainType, token, account = tool_1.INVALID_ADDRESS) {
        ...
        const preSell = async (inputAmount) => {
            ...
            const tokenValueInVault = abi_1.VaultServiceContract.getVaultValue(token, prices[vaultTokenIndex].tokenAmount, tokenPrice);
            const feePoint = abi_1.LPServiceContract.getGlpFeePoint(valueGLP, vaultValue, tokenValueInVault, false, prices[vaultTokenIndex].tokenConfig, vaultTokens[vaultTokenIndex].weight);
