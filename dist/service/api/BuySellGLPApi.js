...
let BuySellGLPApi = class BuySellGLPApi {
    async tokens(chainType, account = undefined) {
        ...
        return vaultTokens.map((it, index) => {
            const feeBasisPoints = new bignumber_js_1.default(tokenConfigs[index].tokenConfig.feeBasisPoints).div(1e4).multipliedBy("100").toFixed();
            const taxBasisPoints = new bignumber_js_1.default(tokenConfigs[index].tokenConfig.taxBasisPoints).div(1e4).multipliedBy("100").toFixed();
