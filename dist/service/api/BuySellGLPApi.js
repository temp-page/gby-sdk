...
let BuySellGLPApi = class BuySellGLPApi {
    ...
    async buyInfo(chainType, token, account = tool_1.INVALID_ADDRESS) {
        ...
        const preBuy = async (inputAmount) => {
            ...
            const tokenValueInVault = abi_1.PortalContract.getVaultValue(token, prices[vaultTokenIndex].tokenAmount, tokenPrice);
            const feePoint = LPServiceContract_1.LPServiceContract.getGlpFeePoint(tokenValue, vaultValue, tokenValueInVault, true, prices[vaultTokenIndex].tokenConfig, vaultTokens[vaultTokenIndex].weight);
            const apy = "10%";
            ...
            const feeAmount = new bignumber_js_1.default(inputValue).multipliedBy(feePoint).div(1e4).toFixed();
            const preBuyGLPInfo = {
    ...
    async sellInfo(chainType, token, account = tool_1.INVALID_ADDRESS) {
        ...
        const preSell = async (inputAmount) => {
            ...
            const tokenValueInVault = abi_1.PortalContract.getVaultValue(token, prices[vaultTokenIndex].tokenAmount, tokenPrice);
            const feePoint = LPServiceContract_1.LPServiceContract.getGlpFeePoint(valueGLP, vaultValue, tokenValueInVault, false, prices[vaultTokenIndex].tokenConfig, vaultTokens[vaultTokenIndex].weight);
