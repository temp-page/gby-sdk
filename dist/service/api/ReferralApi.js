...
let ReferralApi = class ReferralApi {
    async referralInfo(chainType, address) {
        ...
        if (address) {
            const [ClaimShareLogs, IncreaseReferrerShareLogs,] = await Promise.all([
                BaseApi_1.BASE_API.prepGraph((0, gql_1.PrepClaimShareLogsGql)(address)),
                BaseApi_1.BASE_API.prepGraph((0, gql_1.PrepIncreaseReferrerShareLogsGql)(address))
            ]);
            ...
            const [{ shares, code, bindCode }] = await connectInfo.multiCall().callObj({
                shares: vaultServiceContract.multicall_getAccountAllShare(address),
                code: referralServiceContract.multicall_getReferralbyReferrerAddress(address),
                bindCode: referralServiceContract.multicall_getReferralIDByAccountAddress(address)
            });
            const [{ feeDiscount }] = await connectInfo.multiCall().callObj({
                feeDiscount: referralServiceContract.multicall_getReferralFeeDiscount(bindCode),
            });
            {
