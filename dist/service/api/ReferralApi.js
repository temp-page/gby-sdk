    ...
    async referralInfo(chainType, address) {
        const referralInfo = {
            ...
            kolData: {
                ...
                filter: {
                    tokens: [],
                    list(filterTokens, sortBy, sortField) {
                        return [];
                    }
                }
            },
        ...
        if (address) {
            ...
            });
            {
                ...
                }).filter(it => new bignumber_js_1.default(it.amount).comparedTo("0") > 0);
                ...
                referralInfo.kolData.referrals = IncreaseReferrerShareLogs.increaseReferrerShareLogs.map(it => {
                  ...
                  return new BigNumber(b.feesPaid).comparedTo(a.feesPaid)
                })*/
                referralInfo.kolData.filter.tokens = Array.from(new Set([...referralInfo.kolData.referrals.map(it => it.token), ...vaultTokens.map(it => it.token.symbol)]));
                referralInfo.kolData.filter.list = (filterTokens, sortBy, sortField) => {
                    const tokens = filterTokens || [];
                    let list = Array.from(referralInfo.kolData.referrals);
                    if (tokens.length > 0) {
                        const set = new Set(tokens);
                        list = list.filter(it => set.has(it.token));
                    }
                    if (sortField === 'feesPaid') {
                        if (sortBy === 'asc') {
                            list = list.sort((a, b) => {
                                return new bignumber_js_1.default(a[sortField]).comparedTo(b[sortField]);
                            });
                        }
                        else if (sortBy === 'desc') {
                            list = list.sort((a, b) => {
                                return new bignumber_js_1.default(b[sortField]).comparedTo(a[sortField]);
                            });
                        }
                    }
                    return list;
