...
let TradeApi = class TradeApi {
    ...
    preOpenPosition(preOpenPositionParams, tick, targetConfig) {
        ...
        let margin = tool_1.MarketTradeMath.margin(tokenAmount, tokenPrice, preOpenPositionParams.balance.balance.token.decimals);
        // margin
        margin = tool_1.MarketTradeMath.finalMargin(margin, targetConfig.execFeeValue);
        let notionalPositionValue = tool_1.MarketTradeMath.notionalPositionValue(margin, leverage);
        ...
        let openFee = tool_1.MarketTradeMath.openFee(notionalPositionValue, targetConfig.openFeeRate);
        openFee = "0";
        let closeFee = tool_1.MarketTradeMath.closeFee(true, notionalPositionValue, targetConfig.closeFeeRate);
        closeFee = "0";
        const totalFee = tool_1.MarketTradeMath.totalFee(openFee, closeFee, targetConfig.execFeeValue);
        notionalPositionValue = tool_1.MarketTradeMath.finalNetTradeValue(notionalPositionValue, targetConfig.execFeeValue, openFee, closeFee);
