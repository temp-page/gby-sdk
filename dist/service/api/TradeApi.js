...
let TradeApi = class TradeApi {
    ...
    async openPosition(connectInfo, preOpenPositionParams, targetTick, referralCode) {
        ...
        let referralID = ethers6_1.ethers.keccak256(ethers6_1.ethers.AbiCoder.defaultAbiCoder().encode(["string"], [referralCode || ""]));
        if (referralIDByAccountAddress !== tool_1.ZERO_BYTE32) {
            referralID = undefined;
        }
        const transactionEvent = await boostTradeServicesContract.openBoostTrade(isLong, targetId, tokenId, tokenAmount, leverage, takeProfit, priceFeedsUpdateData, referralID);
