...
let TradeApi = class TradeApi {
    ...
    async tradeInfo(chainType, targetName, referralCode) {
        ...
        const tradeInfo = {
            ...
            addMarginInfo: async (positionHash) => {
                ...
                return this.addMarginInfo(chainType, positionData, supportedTokens);
            },
    ...
    preOpenPosition(preOpenPositionParams, tick, targetConfig) {
        ...
        if (preOpenPositionParams.takeProfit.inputType === "price") {
            tp.price = preOpenPositionParams.takeProfit.value;
            const rate = tool_1.MarketTradeMath.takeProfit(isLong, new bignumber_js_1.default(tp.price).multipliedBy(10 ** vo_1.CONST.PRICE_DECIMALS).toFixed(0, bignumber_js_1.default.ROUND_DOWN), margin, targetAmount, targetPrice);
        ...
        if (preOpenPositionParams.takeProfit.inputType === "rate") {
            let val = preOpenPositionParams.takeProfit.value;
            const price = tool_1.MarketTradeMath.takeProfitPrice(isLong, new bignumber_js_1.default(val).multipliedBy(10 ** vo_1.CONST.RATIO_DECIMALS).plus(1e6).toFixed(0, bignumber_js_1.default.ROUND_DOWN), margin, targetAmount, targetPrice);
        ...
        if (preOpenPositionParams.stopLoss.inputType === "price") {
            sl.price = preOpenPositionParams.stopLoss.value;
            const rate = tool_1.MarketTradeMath.stopLossRate(isLong, new bignumber_js_1.default(sl.price).multipliedBy(10 ** vo_1.CONST.PRICE_DECIMALS).toFixed(0, bignumber_js_1.default.ROUND_DOWN), margin, targetAmount, targetPrice);
        ...
        if (preOpenPositionParams.stopLoss.inputType === "rate") {
            const value = preOpenPositionParams.stopLoss.value;
            const price = tool_1.MarketTradeMath.stopLossPrice(isLong, new bignumber_js_1.default(1e6).minus(new bignumber_js_1.default(value).multipliedBy(10 ** vo_1.CONST.RATIO_DECIMALS)).toFixed(0, bignumber_js_1.default.ROUND_DOWN), margin, targetAmount, targetPrice);
            sl.price = new bignumber_js_1.default(price).div(10 ** vo_1.CONST.PRICE_DECIMALS).toFixed(8, bignumber_js_1.default.ROUND_DOWN);
            sl.rate = value;
        }
        if (!(0, tool_1.isNumber)(tp.price) || new bignumber_js_1.default(tp.rate).comparedTo("25") < 0 || new bignumber_js_1.default(tp.rate).comparedTo("300") > 0) {
            tp.error = true;
            if (new bignumber_js_1.default(tp.rate).comparedTo("0") <= 0 || !(0, tool_1.isNumber)(tp.price)) {
                ...
                if (isLong) {
                    if (new bignumber_js_1.default(tp.price).comparedTo(tick.close) < 0) {
                        tp.errorMsg = "Your TP price is ineffective. Please reset the price.";
                    }
                }
                else {
        ...
        if (!(0, tool_1.isNumber)(sl.price) || new bignumber_js_1.default(sl.rate).comparedTo("85") > 0 || new bignumber_js_1.default(sl.rate).comparedTo("0") <= 0) {
            sl.error = true;
            if (new bignumber_js_1.default(sl.rate).comparedTo("0") <= 0 || !(0, tool_1.isNumber)(sl.price)) {
                ...
                if (isLong) {
                    if (new bignumber_js_1.default(sl.price).comparedTo(tick.close) > 0) {
                        tp.errorMsg = "Your SL price is ineffective. Please reset the price.";
                    }
                }
                else {
                    if (new bignumber_js_1.default(sl.price).comparedTo(tick.close) < 0) {
        ...
        const openPosition = {
            ...
            openFeeUSD: openFeeUSD,
            execFeeUSD: execFeeUSD,
            tp,
            sl,
    ...
    async openPosition(connectInfo, openPosition, targetTick, referralCode) {
        ...
        if (targetTick.boost) {
            ...
            transactionEvent = await boostTradeServicesContract.openBoostTrade(isLong, targetId, tokenId, tokenAmount, leverage, takeProfit, priceFeedsUpdateData, referralID);
        }
        else {
            const marketTradeServicesContract = connectInfo.create(contract_1.MarketTradeServicesContract);
            transactionEvent = await marketTradeServicesContract.openMarketTrade(isLong, targetId, tokenId, tokenAmount, leverage, takeProfit, stopLoss, priceFeedsUpdateData, referralID);
        }
    ...
    async addMarginInfo(chainType, positionData, supportedTokens) {
        ...
        const TPSLInfo = {
            ...
            preUpdate(tpParams, slParams) {
                ...
                if (tpParams.inputType === "price") {
                    ...
                    const rate = tool_1.MarketTradeMath.takeProfit(positionData.isLong, new bignumber_js_1.default(tp.price).multipliedBy(10 ** vo_1.CONST.PRICE_DECIMALS).toFixed(0, bignumber_js_1.default.ROUND_DOWN), positionData.sourceData.margin, positionData.sourceData.targetAmount, positionData.sourceData.targetOpenPrice);
                    tp.rate = new bignumber_js_1.default(rate).minus(1e6).div(10 ** vo_1.CONST.RATIO_DECIMALS).toFixed(vo_1.CONST.RATIO_DECIMALS, bignumber_js_1.default.ROUND_DOWN);
                }
                if (tpParams.inputType === "rate") {
                    let val = tpParams.value;
                    const price = tool_1.MarketTradeMath.takeProfitPrice(positionData.isLong, new bignumber_js_1.default(val).multipliedBy(10 ** vo_1.CONST.RATIO_DECIMALS).plus(1e6).toFixed(0, bignumber_js_1.default.ROUND_DOWN), positionData.sourceData.margin, positionData.sourceData.targetAmount, positionData.sourceData.targetOpenPrice);
                    tp.price = new bignumber_js_1.default(price).div(10 ** vo_1.CONST.PRICE_DECIMALS).toFixed(8, bignumber_js_1.default.ROUND_DOWN);
                    tp.rate = val;
                }
                const sl = {
                ...
                if (slParams.inputType === "price") {
                    ...
                    const rate = tool_1.MarketTradeMath.stopLossRate(positionData.isLong, new bignumber_js_1.default(sl.price).multipliedBy(10 ** vo_1.CONST.PRICE_DECIMALS).toFixed(0, bignumber_js_1.default.ROUND_DOWN), positionData.sourceData.margin, positionData.sourceData.targetAmount, positionData.sourceData.targetOpenPrice);
                    sl.rate = new bignumber_js_1.default(1e6).minus(rate).div(10 ** vo_1.CONST.RATIO_DECIMALS).toFixed(vo_1.CONST.RATIO_DECIMALS, bignumber_js_1.default.ROUND_DOWN);
                }
                if (slParams.inputType === "rate") {
                    const value = slParams.value;
                    const price = tool_1.MarketTradeMath.stopLossPrice(positionData.isLong, new bignumber_js_1.default(1e6).minus(new bignumber_js_1.default(value).multipliedBy(10 ** vo_1.CONST.RATIO_DECIMALS)).toFixed(0, bignumber_js_1.default.ROUND_DOWN), positionData.sourceData.margin, positionData.sourceData.targetAmount, positionData.sourceData.targetOpenPrice);
                    sl.price = new bignumber_js_1.default(price).div(10 ** vo_1.CONST.PRICE_DECIMALS).toFixed(8, bignumber_js_1.default.ROUND_DOWN);
                    sl.rate = value;
                }
                if (!(0, tool_1.isNumber)(tp.price) || new bignumber_js_1.default(tp.rate).comparedTo("0") <= 0 || new bignumber_js_1.default(tp.rate).comparedTo("300") > 0) {
                    tp.error = true;
                    if (new bignumber_js_1.default(tp.rate).comparedTo("0") <= 0 || !(0, tool_1.isNumber)(tp.price)) {
                        tp.errorMsg = "Your TP price is ineffective. Please reset the price.";
                    }
                        ...
                        if (positionData.isLong) {
                            if (new bignumber_js_1.default(tp.price).comparedTo(targetTick.close) < 0) {
                                tp.errorMsg = "Your TP price is ineffective. Please reset the price.";
                            }
                        }
                        else {
                ...
                if (!(0, tool_1.isNumber)(sl.price) || new bignumber_js_1.default(sl.rate).comparedTo("85") > 0 || new bignumber_js_1.default(sl.rate).comparedTo("0") <= 0) {
                    ...
                    if (new bignumber_js_1.default(sl.rate).comparedTo("0") <= 0 || !(0, tool_1.isNumber)(sl.price)) {
                        ...
                        if (positionData.isLong) {
                            if (new bignumber_js_1.default(sl.price).comparedTo(targetTick.close) > 0) {
                                tp.errorMsg = "Your SL price is ineffective. Please reset the price.";
                            }
                        }
                        else {
