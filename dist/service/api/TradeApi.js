...
const TradeEventBus_1 = require("../tool/kline/TradeEventBus");
const ServiceWs_1 = require("./ServiceWs");
const BoostTradeServicesContract_1 = require("../abi/BoostTradeServicesContract");
...
let TradeApi = class TradeApi {
    ...
    startPositionEvent() {
        ...
        timeIdManger.positionUpdate = setTimeout(() => {
            try {
                TradeEventBus_1.tradeEventBus.emitAll(TradeEventBus_1.TradeEventBus.SYS_TOPIC_POSITIONS, []);
            }
            finally {
    ...
    async tradeInfo(chainType, targetName, referral) {
        this.startEvent();
        const addressInfo = BaseApi_1.BASE_API.address();
        const chainInfo = addressInfo.getChainInfo(chainType);
        const connectInfo = BaseApi_1.BASE_API.connectInfo(chainInfo);
        ...
        const [{ getTargetConfig, getFundingFeeRate }] = await connectInfo.multiCall().callObj({
            getTargetConfig: targetServiceContract.multicall_getTargetConfig(target.targetId),
            getFundingFeeRate: targetServiceContract.multicall_getFundingFeeRate(target.targetId),
        });
        const targetConfig = {
        ...
        const TOPIC_PRE_OPEN_POSITION_ID = `${TradeEventBus_1.TradeEventBus.TOPIC_PRE_OPEN_POSITION}_${targetName}`;
        const TOPIC_PAIR_PRICE_ID = `${TradeEventBus_1.TradeEventBus.TOPIC_PAIR_PRICE}_${targetName}`;
        const TOPIC_PAIR_KLINE_ID = `${TradeEventBus_1.TradeEventBus.TOPIC_KLINE}_${targetName}`;
        const cache = {
        ...
        async function getKlineData(period, topic) {
            const klineData = await ApiProvider_1.apiProvider.serverApi().klines(targetName, period, 0, 0, topic ? 1 : 1000);
        ...
        cache.eventIds.add(TradeEventBus_1.TradeEventBus.SYS_TOPIC_TICK_DATA_UPDATE);
        TradeEventBus_1.tradeEventBus.resetOn(TradeEventBus_1.TradeEventBus.SYS_TOPIC_TICK_DATA_UPDATE, (tickData) => {
            ...
            TradeEventBus_1.tradeEventBus.emitAll(TradeEventBus_1.TradeEventBus.SYS_TOPIC_TICKS_UPDATE, targetTicks);
        });
        const tradeInfo = {
            topicPreOpenPosition: (eventId, func) => {
                const eventName = `${TOPIC_PRE_OPEN_POSITION_ID}_${eventId}`;
                cache.eventIds.add(eventName);
                TradeEventBus_1.tradeEventBus.resetOn(eventName, func);
            },
            ...
            topicAllPrice: (eventId, func) => {
                const eventName = `${TradeEventBus_1.TradeEventBus.TOPIC_ALL_PRICE}_${eventId}`;
                cache.eventIds.add(eventName);
                TradeEventBus_1.tradeEventBus.resetOn(eventName, func);
            },
            ...
            topicPositions: (eventId, func) => {
                const eventName = `${TradeEventBus_1.TradeEventBus.TOPIC_POSITIONS}_${eventId}`;
                cache.eventIds.add(eventName);
                TradeEventBus_1.tradeEventBus.resetOn(eventName, func);
            },
            ...
            open: async (connectInfo) => {
                ...
                return this.openPosition(connectInfo, cache.preOpenPositionParams, tradeInfo.targetsTick, referral);
            },
            ...
            closeTopic() {
                cache.eventIds.forEach(it => {
                    TradeEventBus_1.tradeEventBus.resetOff(it);
                });
                TradeEventBus_1.tradeEventBus.removeAll(TOPIC_PRE_OPEN_POSITION_ID);
                TradeEventBus_1.tradeEventBus.removeAll(TOPIC_PAIR_PRICE_ID);
            }
        };
        const SYS_TOPIC_TICKS_UPDATE = (data) => {
            ...
            TradeEventBus_1.tradeEventBus.emitAll(TradeEventBus_1.TradeEventBus.TOPIC_ALL_PRICE, tradeInfo.targetsTicks);
            TradeEventBus_1.tradeEventBus.emitAll(TOPIC_PAIR_PRICE_ID, tradeInfo.targetsTick);
            if (cache.preOpenPositionParams) {
                const preOpenPosition = tradeInfo.preOpenPosition(cache.preOpenPositionParams);
                TradeEventBus_1.tradeEventBus.emitAll(TOPIC_PRE_OPEN_POSITION_ID, preOpenPosition);
            }
            if (cache.currentPositions) {
                cache.currentPositions.forEach(it => {
                    this.updatePnl(it, data);
                });
                TradeEventBus_1.tradeEventBus.emitAll(TradeEventBus_1.TradeEventBus.TOPIC_POSITIONS, cache.currentPositions);
            }
            if (cache.period) {
                getKlineData(cache.period, true)
                    .then(klineData => {
                    TradeEventBus_1.tradeEventBus.emitAll(TOPIC_PAIR_KLINE_ID, klineData);
                });
            }
        };
        ...
        // 订阅处理
        TradeEventBus_1.tradeEventBus.resetOn(TradeEventBus_1.TradeEventBus.SYS_TOPIC_TICKS_UPDATE, SYS_TOPIC_TICKS_UPDATE);
        TradeEventBus_1.tradeEventBus.resetOn(TradeEventBus_1.TradeEventBus.SYS_TOPIC_POSITIONS, SYS_TOPIC_POSITIONS);
        const oldClose = tradeInfo.closeTopic;
        tradeInfo.closeTopic = () => {
            oldClose();
            TradeEventBus_1.tradeEventBus.off(TradeEventBus_1.TradeEventBus.SYS_TOPIC_TICKS_UPDATE, SYS_TOPIC_TICKS_UPDATE);
            TradeEventBus_1.tradeEventBus.off(TradeEventBus_1.TradeEventBus.SYS_TOPIC_POSITIONS, SYS_TOPIC_POSITIONS);
    ...
    async positions(tickPrices, vaultTokens, targets, chainType, userAddress) {
        ...
        const positionServiceContract = connectInfo.create(abi_1.PositionServiceContract);
        const targetServiceContract = connectInfo.create(TargetServiceContract_1.TargetServiceContract);
        const multiCallContract = connectInfo.create(abi_1.MultiCallContract);
        const result = [];
        let page = 0;
        const pageSize = 1000;
        const shapeWithLabels = targets.map((it) => {
            return {
                key: it.base,
                fundingFeeRate: targetServiceContract.multicall_getFundingFeeRate(it.targetId),
                openInterest: targetServiceContract.multicall_getOpenInterest(it.targetId),
            };
        });
    ...
    updateFundingFee(positionData, blockTime, targetConfigMap) {
        ...
        const fundingFeeAmount = new bignumber_js_1.default(fundingFee).div(10 ** vo_1.CONST.QTY_DECIMALS).div(10 ** vo_1.CONST.PRICE_DECIMALS).toFixed();
        positionData.fundingFee = fundingFeeAmount;
    }
    updatePnl(positionData, tickPrices) {
        ...
        positionData.markPrice = tickPrice.close;
        positionData.qty = new bignumber_js_1.default(positionData.size).multipliedBy(tickPrice.close).toFixed();
    }
    positionRecordAbiResultToPositionData(positionRecordAbiResult, pairs, tokens) {
        ...
        const positionData = {
            ...
            fundingFee: new bignumber_js_1.default(positionRecordAbiResult.fundingFee).div(10 ** vo_1.CONST.QTY_DECIMALS).div(10 ** vo_1.CONST.PRICE_DECIMALS).toFixed(),
    ...
    preOpenPosition(preOpenPositionParams, tick, targetConfig) {
        ...
        // const feeByTokenAmount = MarketTradeMath.feeByTokenAmount(
        //   openFee,
        //   closeFee,
        //   preOpenPositionParams.balance.balance.token.decimals,
        //   tokenPrice
        // )
        ...
        const openFeeAmount = new bignumber_js_1.default(openFee).multipliedBy(10 ** preOpenPositionParams.balance.balance.token.decimals).multipliedBy(tokenPrice).div(10 ** vo_1.CONST.QTY_DECIMALS)
            .dp(0, bignumber_js_1.default.ROUND_DOWN)
        ...
        const openPosition = {
            ...
            openFeeUSD: new bignumber_js_1.default(openFee).div(10 ** vo_1.CONST.QTY_DECIMALS).div(10 ** vo_1.CONST.PRICE_DECIMALS).toFixed(8, bignumber_js_1.default.ROUND_DOWN),
            execFeeUSD: new bignumber_js_1.default(targetConfig.execFeeValue).div(10 ** vo_1.CONST.QTY_DECIMALS).div(10 ** vo_1.CONST.PRICE_DECIMALS).toFixed(8, bignumber_js_1.default.ROUND_DOWN)
        };
        return openPosition;
    }
    async openPosition(connectInfo, preOpenPositionParams, targetTick, referral) {
        const boostTradeServicesContract = connectInfo.create(BoostTradeServicesContract_1.BoostTradeServicesContract);
        const isLong = preOpenPositionParams.isLong;
        ...
        const leverage = new bignumber_js_1.default(preOpenPositionParams.leverage).multipliedBy(1e2).multipliedBy(1e4).toFixed(0, bignumber_js_1.default.ROUND_DOWN);
        const takeProfit = new bignumber_js_1.default(preOpenPositionParams.takeProfit).multipliedBy(1e4).plus(1e6).toFixed(0, bignumber_js_1.default.ROUND_DOWN);
        ...
        const transactionEvent = await boostTradeServicesContract.openBoostTrade(isLong, targetId, tokenId, tokenAmount, leverage, takeProfit, priceFeedsUpdateData, referral || "0x0" // TODO referral
        );
    ...
    async closePosition(positionHash, connectInfo) {
        ...
        const transactionEvent = await boostTradeServicesContract.closeBoostTrade(positionHash, priceFeedsUpdateData);
