...
let TradeApi = TradeApi_1 = class TradeApi {
    ...
    async tradeInfo(chainType, targetName, referralCode) {
        ...
        const tradeInfo = {
            ...
            positions: async (account, updateAccount = true) => {
                if (updateAccount) {
                    cache.topicAccount = account;
                }
                const positions = cache.topicAccount ? await this.positions(tradeInfo.targetsTickMap, vaultTokens, targets, chainType, cache.topicAccount) : [];
        ...
        const SYS_TOPIC_POSITIONS = async () => {
            if (cache.topicAccount) {
                await tradeInfo.positions(cache.topicAccount, false);
                tool_1.tradeEventBus.emitAll(tool_1.TradeEventBus.TOPIC_POSITIONS, cache.currentPositions);
            }
        };
        // 订阅处理
    ...
    async positionHistories(chainType, vaultTokens, targets, userAddress, page = 1, pageSize = 1000) {
        ...
        return {
            ...
            list: positionHistories.map(it => {
                ...
                const positionData = this.positionRecordAbiResultToPositionData(it, targetsMap, vaultTokenMap, undefined);
    ...
    async positions(tickPrices, vaultTokens, targets, chainType, userAddress) {
        ...
        return result.map(it => {
            const positionData = this.positionRecordAbiResultToPositionData(it, targetsMap, vaultTokenMap, (pd) => {
                this.updatePnl(pd, tickPrices);
            });
    ...
    positionRecordAbiResultToPositionData(positionRecordAbiResult, pairs, tokens, pnlFunc) {
        ...
        if (pnlFunc) {
            pnlFunc(positionData);
        }
        const netProfit = new bignumber_js_1.default(positionRecordAbiResult.tokenClosePrice).eq("0") ? positionData.pnl : new bignumber_js_1.default(positionRecordAbiResult.pnl)
            .div(10 ** vo_1.CONST.QTY_DECIMALS)
            .div(10 ** vo_1.CONST.PRICE_DECIMALS)
            .toFixed();
        const netProfitToken = new bignumber_js_1.default(positionRecordAbiResult.tokenClosePrice).eq("0") ? positionData.pnl : new bignumber_js_1.default(positionRecordAbiResult.pnl).multipliedBy(10 ** token.token.decimals)
        ...
        positionData.netProfit = new bignumber_js_1.default(netProfit).minus(positionData.openFee).minus(positionData.execFee).minus(positionData.closeFee).toFixed();
        positionData.netProfitToken = new bignumber_js_1.default(netProfitToken).minus(positionData.openFeeToken).minus(positionData.execFeeToken).minus(positionData.closeFeeToken).toFixed();
        if (new bignumber_js_1.default(positionData.stopLossPrice).eq("0") || new bignumber_js_1.default(positionData.stopLossPrice).eq(new bignumber_js_1.default(tool_1.MAXIMUM_U256).div(10 ** vo_1.CONST.PRICE_DECIMALS).toFixed(8, bignumber_js_1.default.ROUND_DOWN))) {
            positionData.stopLossPrice = "none";
        }
        return positionData;
    ...
    preOpenPosition(preOpenPositionParams, tick, targetConfig) {
        ...
        TradeApi_1.checkSlError(sl, isLong, tick.boost, tick.close, preOpenPositionParams.stopLoss, {
            margin,
            targetAmount,
            targetPrice
        });
        const openPosition = {
            ...
            liqPrice: liqPrice,
            openFee: new bignumber_js_1.default(openFeeAmount).toFixed(2, bignumber_js_1.default.ROUND_HALF_UP),
            execFee: new bignumber_js_1.default(execFeeAmount).toFixed(2, bignumber_js_1.default.ROUND_HALF_UP),
    ...
    async openPosition(connectInfo, openPosition, targetTick, referralCode) {
        ...
        const getUpdateFee = await pythContract.getUpdateFee(priceFeedsUpdateData);
        const value = new bignumber_js_1.default(getUpdateFee).toFixed();
    ...
    async closePosition(positionHash, positionType, connectInfo) {
        ...
        if (parseInt(positionType) === vo_1.CONST.POSITION_TYPE_MARKET) {
            const marketTradeServicesContract = connectInfo.create(contract_1.MarketTradeServicesContract);
            transactionEvent = await marketTradeServicesContract.closeMarketTrade(positionHash, priceFeedsUpdateData, value);
        }
        else {
            const boostTradeServicesContract = connectInfo.create(contract_1.BoostTradeServicesContract);
            transactionEvent = await boostTradeServicesContract.closeBoostTrade(positionHash, priceFeedsUpdateData, value);
        }
    ...
    async TPSLInfo(tradeInfo, positionData) {
        ...
        const TPSLInfo = {
            ...
            preUpdate(tpParams, slParams) {
                ...
                TradeApi_1.checkSlError(sl, positionData.isLong, targetTick.boost, targetTick.close, slParams, {
                    margin: positionData.sourceData.margin,
                    targetAmount: positionData.sourceData.targetAmount,
                    targetPrice: positionData.sourceData.targetOpenPrice,
                });
            ...
            update(tp, sl, connectInfo) {
                ...
                let targetTick = tradeInfo.targetsTickMap[positionData.base];
                if (targetTick.boost) {
                    return positionServiceContract.updatePostionTakeProfit(positionHash, new bignumber_js_1.default(result.tp.rate).multipliedBy(1e4).plus(1e6).toFixed());
                }
                else {
    ...
    static checkSlError(sl, isLong, boost, closePrice, slParams, params) {
    ...
    static checkTpError(tp, isLong, closePrice, tpParams, params) {
        ...
        else if (new bignumber_js_1.default(tp.rate).comparedTo("300") > 0 || new bignumber_js_1.default(tp.rate).comparedTo("50") < 0) {
            tp.errorMsg = "Take Profit must be within 50%-300%";
        }
        else {
            if (isLong) {
...
__decorate([
    (0, tool_1.MethodCache)("TradeApi.pairs.${args[0]}", 30 * 1000),
