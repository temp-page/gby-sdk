...
let TradeApi = class TradeApi {
    ...
    async tradeInfo(chainType, targetName) {
        ...
        const [{ getTargetConfig, }] = await connectInfo.multiCall().callObj({
    ...
    updateFundingFee(positionData, blockTime, targetConfigMap) {
        ...
        const openInterest = targetConfigMap[positionData.sourceData.target].openInterest;
        ...
        if (period.gt("0")) {
            const fundingFeeRate = new bignumber_js_1.default(targetConfigMap[positionData.sourceData.target].fundingFeeRate);
            const newAccFundingFeeLong = new bignumber_js_1.default(accFundingFeeLong).plus(fundingFeeRate.multipliedBy(period));
        ...
        positionData.fundingFee = new bignumber_js_1.default(fundingFee).div(10 ** vo_1.CONST.QTY_DECIMALS).div(10 ** vo_1.CONST.PRICE_DECIMALS).toFixed();
    }
    ...
    positionRecordAbiResultToPositionData(positionRecordAbiResult, pairs, tokens) {
        ...
        const positionData = {
            ...
            duration: new bignumber_js_1.default(positionRecordAbiResult.closeTimeStamp).eq("0") ? new bignumber_js_1.default(new Date().getTime() / 1000).minus(new bignumber_js_1.default(positionRecordAbiResult.openTimeStamp)).toFixed() :
                new bignumber_js_1.default(positionRecordAbiResult.closeTimeStamp).minus(new bignumber_js_1.default(positionRecordAbiResult.openTimeStamp)).toFixed(0, bignumber_js_1.default.ROUND_DOWN),
            token: token.token,
            netProfit: new bignumber_js_1.default(positionRecordAbiResult.pnl).div(10 ** vo_1.CONST.QTY_DECIMALS).div(10 ** vo_1.CONST.PRICE_DECIMALS).toFixed(),
            netProfitToken: new bignumber_js_1.default(positionRecordAbiResult.pnl).multipliedBy(10 ** token.token.decimals)
    ...
    preOpenPosition(preOpenPositionParams, tick, targetConfig) {
        ...
        let notionalPositionValue = tool_1.MarketTradeMath.notionalPositionValue(tokenAmount, leverage, tokenPrice, preOpenPositionParams.balance.balance.token.decimals);
        ...
        const openFee = tool_1.MarketTradeMath.openFee(notionalPositionValue, targetConfig.openFeeRate);
        const closeFee = tool_1.MarketTradeMath.closeFee(true, notionalPositionValue, targetConfig.closeFeeRate);
        const totalFee = tool_1.MarketTradeMath.totalFee(openFee, closeFee, targetConfig.execFeeValue);
        // margin
        margin = tool_1.MarketTradeMath.finalMargin(margin, openFee, closeFee, targetConfig.execFeeValue);
