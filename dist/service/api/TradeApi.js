...
let TradeApi = class TradeApi {
    ...
    async tradeInfo(chainType, targetName, referralCode) {
        ...
        async function getKlineData(period, topic) {
            const klineData = await ApiProvider_1.apiProvider.serverApi().klines(targetTick.base, period, 0, 0, topic ? 1 : 1000);
    ...
    preOpenPosition(preOpenPositionParams, tick, targetConfig) {
        ...
        let openFee = "0";
        // const closeFee = "0"
        let type = TargetType.boost;
        if (new bignumber_js_1.default(leverage).comparedTo("500") < 0) {
    ...
    async openPosition(connectInfo, openPosition, targetTick, referralCode) {
        ...
        if (targetTick.boost) {
            ...
            const stopLoss = new bignumber_js_1.default("100").multipliedBy(1e2).multipliedBy(1e4).toFixed(0, bignumber_js_1.default.ROUND_DOWN);
    ...
    async tokens(chainType, supportedTokens, account = undefined) {
        ...
        const supportedTokenSet = new Set(supportedTokens);
        vaultTokens.forEach(it => {
            if (!supportedTokenSet.has(it.assetID)) {
                supportedTokenSet.add(it.assetID);
            }
        });
        ...
        }).filter(it => supportedTokenSet.has(it.tokenId));
    }
};
