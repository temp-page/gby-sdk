...
let TradeApi = class TradeApi {
    async pairs(chainType) {
        ...
        const targetServiceContract = connectInfo.create(abi_1.TargetServiceContract);
    ...
    async tradeInfo(chainType, targetName, referralCode) {
        ...
        const connectInfo = BaseApi_1.BASE_API.connectInfo(chainInfo);
        const targetServiceContract = connectInfo.create(abi_1.TargetServiceContract);
        const referralServiceContract = connectInfo.create(abi_1.ReferralServiceContract);
        ...
        const referralId = (0, ethers6_1.encodeBytes32String)(referralCode || "");
        const [{ getTargetConfig, referralAddress, referralIDByAccountAddress }] = await connectInfo.multiCall().callObj({
            getTargetConfig: targetServiceContract.multicall_getTargetConfig(target.targetId),
            referralAddress: referralServiceContract.multicall_getReferrerAddressByReferralID(referralId),
            referralIDByAccountAddress: referralServiceContract.multicall_getReferralIDByAccountAddress(connectInfo.account),
        });
        if (referralAddress === tool_1.ZERO_ADDRESS || referralIDByAccountAddress !== tool_1.ZERO_BYTE32) {
        ...
        const tradeInfo = {
            ...
            open: async (connectInfo) => {
                ...
                return this.openPosition(connectInfo, cache.preOpenPositionParams, tradeInfo.targetsTick, referralCode);
            },
    ...
    async openPosition(connectInfo, preOpenPositionParams, targetTick, referralCode) {
        ...
        const transactionEvent = await boostTradeServicesContract.openBoostTrade(isLong, targetId, tokenId, tokenAmount, leverage, takeProfit, priceFeedsUpdateData, referralCode // TODO referral
        );
