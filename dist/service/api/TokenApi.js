...
let TokenApi = class TokenApi {
    async vaultTokenList(chainType) {
        ...
        const [{ listTokens }] = await connectInfo.multiCall().callObj({
            listTokens: vaultServiceContract.multicall.listTokens()
        });
        const [tokens,] = await Promise.all([
            this.batchGetTokens(chainInfo, listTokens.tokenAddress)
        ]);
        const vaultTokens = listTokens.tokenAddress.map((it, index) => {
            const vaultToken = {
                address: it,
                assetID: listTokens.assetID[index],
                token: tokens[it],
                weight: listTokens.targetWeights[index],
                tokenAmount: listTokens.tokenAmounts[index],
                priceId: listTokens.priceID[index]
            };
            return vaultToken;
        });
        return vaultTokens;
    ...
    async getTokenByContract(chainInfo, addresses) {
        try {
            ...
            return tokenInfos
                ...
                .map((it) => {
                return new tool_1.Token(chainInfo.chainId, it.address, it.decimals, it.symbol, it.name, `${addressInfo.baseApiUrl}/static/${it.symbol.toLowerCase()}.png`);
            });
        }
        catch (e) {
...
__decorate([
    (0, tool_1.MethodCache)("TokenApi.vaultTokenList.${args[0]}", 3 * 1000),
