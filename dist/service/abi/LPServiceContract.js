...
const PythContract_1 = require("./PythContract");
let LPServiceContract = class LPServiceContract extends BaseAbi_1.BaseAbi {
    ...
    async mintALP(assetID, amountToken, acceptableMinGlpAmount, priceUpdateData) {
        const pythContract = this.connectInfo.create(PythContract_1.PythContract);
        const value = await pythContract.getUpdateFee(priceUpdateData);
        return await this.connectInfo
            .tx()
            .sendContractTransaction(this.contract, 'mintALP', [assetID, amountToken, acceptableMinGlpAmount, priceUpdateData], { value });
    }
    async burnALP(assetID, amountGLP, acceptableMinTokenAmount, priceUpdateData) {
        return await this.connectInfo
            .tx()
            .sendContractTransaction(this.contract, 'burnALP', [assetID, amountGLP, acceptableMinTokenAmount, priceUpdateData]);
    }
