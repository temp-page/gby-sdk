...
// codegen for src/abi/TradeRewards.json time 2025-06-09T01:45:29.075Z
let TradeRewardsContract = class TradeRewardsContract extends BaseAbi_1.BaseAbi {
    constructor(connectInfo) {
        super(connectInfo, connectInfo.chainInfo().GBU, index_1.TradeRewardsAbi);
    ...
    // GET isClaimed(uint256, address) view returns(bool)
    isClaimed(round, account) {
        return this.connectInfo.multiCall().singleCallObj(this.multicall.isClaimed(round, account));
    }
};
...
class TradeRewardsMultiCall {
    ...
    isClaimed(round, account) {
        return this.mulContract.isClaimed(round, account);
    }
}
